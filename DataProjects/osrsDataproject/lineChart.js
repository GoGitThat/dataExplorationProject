...
export default class Linechart extends PureComponent {
  ...
  render() {
    ...
    for (const [index, value] of this.props.ydataKeys.entries()) {
      lines.push(<Line yAxisId={this.props.axes[index]} key={index} strokeWidth={1.25}  dot={false} type="monotone"
      dataKey={this.props.ydataKeys[index]} stroke={this.props.lineStrokes[index]}
      tick={{fontSize: this.props.fontSize, fill:"#CED8D8"}}/>);
    }
    if(this.props.axes.includes("left")){
      axes.push(<YAxis yAxisId="left"
      label={{ value: this.props.ylabelleft, angle: -90, position: 'center', fontSize: this.props.yfontSize, fill:"#CED8D8", dx: -1*this.props.ymargin}}
    ...
    if(this.props.axes.includes("right")){
      ...
      label={{ value: this.props.ylabelright, angle: 90, position: 'center', fontSize: this.props.yfontSize, fill:"#CED8D8", dx: 1*this.props.ymargin}}
      tick={{fontSize: this.props.fontSize, fill:"#CED8D8"}} tickFormatter={DataFormater}/>);
    }
    return (
